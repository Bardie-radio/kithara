...
Source module **harness** library for Bardie hosts (Kithara today; external hosts later).
...
```csharp
services.AddSourceModuleHarness(registerModuleChannel: false);
// when Auth harness already called AddModuleChannel — avoid double-register
// host must register IBlobStorage
```

Pack: `dotnet pack libs/Bardie.Harness.Source/Bardie.Harness.Source.csproj -c Release`
