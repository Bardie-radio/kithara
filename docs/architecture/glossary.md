...
| **Auth harness** | Auth router | Discovery merge, opaque Authenticate/Refresh/UpdateUserBinding routing, login JWT verify (JWKS), guest-code exchange + ephemeral guest user/JWT mint, `SeedAdminBinding`, join secrets |
